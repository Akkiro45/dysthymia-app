...
import { CLOCK, DOUBLE_LEFT_ARROW, DOUBLE_RIGHT_ARROW } from '../../../util/icons';
import { BLUE, GRAY1, PURPLE } from '../../../util/color';
import { fetchStats, resetError, cleanStats } from '../../../store/actions/index';
import Loading from '../../../components/UI/Loading/Loading';
import { miliToTime, miliToHoure } from '../../../util/util';

class ScreenOnTime extends Component {
  state = {
    refreshing: false,
    page: 1,
    finished: -1,
  ...
  componentDidMount() {
    this.props.onFetchStatsStart(this.props.pageNumber, this.props.pageSize, 'screenOnTime', this.props.token, this.props.route.params.initialData);
  }
  ...
  setData = (page, stats, isFinished) => {
    ...
    if(stats.length < 7 && !isFinished) {
      this.props.onFetchStatsStart(this.props.pageNumber, this.props.pageSize, 'screenOnTime', this.props.token);
    } else {
      let labels = [];
      let dataPoints = [];
      let total = 0;
      ...
      stats.forEach(el => {
        labels.push(el.day);
        time = el.time ? el.time : 0;
        total += time;
        time = miliToHoure(time);
      ...
      this.setState({ 
        ...
        data, 
        total, 
        currTime: stats[l-1].time ? stats[l-1].time : 0, 
  ...
  onRefresh = () => {
    ...
    this.props.onFetchStatsStart(1, this.props.pageSize, 'screenOnTime', this.props.token, this.props.route.params.initialData);
  }
    ...
    if((!this.props.loading && this.props.stats.length > 0) && this.state.data ) {
      ren = (
        <View>
          ...
          <Text text={`${miliToTime(this.state.currTime)}`} type='h4' style={{ marginVertical: 30 }} />

          <LineChart 
            data={this.state.data}
            onDataPointClick={(data) => {
              this.setState({ 
                currTime: this.state.stats[data.index].time ? this.state.stats[data.index].time : 0, 
          ...
          <Text text={`${miliToTime(this.state.total)} this week!`} type='h5' style={{ marginVertical: 20, fontFamily: 'Rubik-Light' }} />
        </View>
      );
    } else {
      ren = (
      ...
      >
        <View style={style.header} >
          <View style={style.title} >
            <View>
              <Text text='Screen On Time' type='h3' style={{ color: BLUE }} />
            </View>
...
export default connect(mapStateToProps, mapDispatchToProps)(ScreenOnTime);