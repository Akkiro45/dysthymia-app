...
class UnlockCounter extends Component {
  ...
  componentDidMount() {
    this.props.onFetchStatsStart(this.props.pageNumber, this.props.pageSize, 'unlocks', this.props.token, this.props.route.params.initialData);
  }
  ...
  setData = (page, stats, isFinished) => {
    ...
    if(stats.length < 7 && !isFinished) {
      this.props.onFetchStatsStart(this.props.pageNumber, this.props.pageSize, 'unlocks', this.props.token);
    } else {
      let labels = [];
      let dataPoints = [];
      let total = 0;
  ...
  onRefresh = () => {
    ...
    this.props.onFetchStatsStart(1, this.props.pageSize, 'unlocks', this.props.token, this.props.route.params.initialData);
  }
    ...
    if((!this.props.loading && this.props.stats.length > 0) && this.state.data ) {
      ren = (
        <View>
          ...
          <Text text={`${this.state.currCount} Unlocks`} type='h4' style={{ marginVertical: 30 }} />
          ...
          <Text text={`${this.state.total} unlocks this week!`} type='h5' style={{ marginVertical: 20, fontFamily: 'Rubik-Light' }} />
        </View>
      );
    } else {
      ren = (
      ...
      >
        <View style={style.header} >
          <View style={style.title} >
            <View>
              <Text text='Unlock Counter' type='h3' style={{ color: BLUE }} />
            </View>
            <View style={style.icon} >
              <Icon name={UNLOCK} size={30} color={'#000'} />
            </View>
          </View>
        </View>
        {ren}
...
export default connect(mapStateToProps, mapDispatchToProps)(UnlockCounter);